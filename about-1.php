...
<script>
    ...
    // ---- NOTICES DATA: add your notices here ----
    const notices = [];

    // ---- RENDER NOTICES ----
    const noticeBoardBody = document.getElementById("notice-board-body");
    if (notices.length === 0) {
        noticeBoardBody.innerHTML = '<p class="notice-empty">No notices at this time.</p>';
    } else {
        noticeBoardBody.innerHTML = notices.map(n => `
    ...
    const selInfo   = document.getElementById("sel-event-text");
    ...
    function renderCalendar() {
        ...
        for (let i = 0; i < 6; i++) {
            ...
            for (let j = 0; j < 7; j++) {
                if (i === 0 && j < firstDay) {
                    html += `<td class="muted">${prevLastDate - firstDay + j + 1}</td>`;
                } else if (count > lastDate) {
                    html += `<td class="muted">${extra++}</td>`;
                } else {
                    const isToday =
                        count === today.getDate() &&
                        m === today.getMonth() &&
                    ...
                    const dateKey = `${y}-${pad(m + 1)}-${pad(count)}`;
                    const hasEvent = calendarEvents[dateKey] ? 'has-event' : '';

                    html += `<td class="${isToday ? 'today' : ''} ${hasEvent}" data-date="${dateKey}">${count}</td>`;
                    count++;
                }
            }
            html += '</tr>';
            if (count > lastDate && i >= 3) break;
        }
        ...
        calBody.querySelectorAll('td[data-date]').forEach(td => {
            td.addEventListener('click', function () {
                ...
                document.querySelector('.sel-date-label').textContent = label;

                if (calendarEvents[key]) {
                    selInfo.textContent = `🗓 ${calendarEvents[key].title} — ${calendarEvents[key].desc}`;
                    selInfo.classList.remove('no-event');
                } else {
                    selInfo.textContent = 'No events on this date.';
                    selInfo.classList.add('no-event');
                }
            });
        });
    }
    ...
    function resetSelected() {
        document.querySelector('.sel-date-label').textContent = 'Selected Date';
        selInfo.textContent = 'Click a date to see events';
        selInfo.classList.add('no-event');
    }
